...
namespace VolumetricFogAndMist {
				...
				public class VolumetricFogInspector : Editor {
								...
								public override void OnInspectorGUI () {
												...
												if (_fog.hasCamera) {
																...
																EditorGUILayout.BeginHorizontal ();
																GUILayout.Label (new GUIContent ("Fog Renderer", "Assign here a Volumetric Fog script attached to the camera."), GUILayout.Width (130));
																_fog.fogRenderer = (VolumetricFog)EditorGUILayout.ObjectField (_fog.fogRenderer, typeof(VolumetricFog), true);
																EditorGUILayout.EndHorizontal ();
												}

												EditorGUILayout.BeginHorizontal ();
												GUILayout.Label (new GUIContent ("Legacy Presets", "Load a factory preset. This is deprecated and profiles should be used instead."), GUILayout.Width (130));
												_fog.preset = (FOG_PRESET)EditorGUILayout.EnumPopup (_fog.preset);
												EditorGUILayout.EndHorizontal ();

												EditorGUILayout.BeginHorizontal ();
												GUILayout.Label (new GUIContent ("Profile", "Create or load stored presets."), GUILayout.Width (130));
												_fog.profile = (VolumetricFogProfile)EditorGUILayout.ObjectField (_fog.profile, typeof(VolumetricFogProfile), false);
												...
												if (_fog.hasCamera) {
																...
																EditorGUILayout.BeginHorizontal ();
																GUILayout.Label (new GUIContent ("Compute Depth", "Computes depth for some objects that don't write to z-buffer, such as tree billboards. Enable only if neccesary."), GUILayout.Width (130));
                if (IsFeatureEnabled(VolumetricFog.SKW_FOG_COMPUTE_DEPTH))
                {
                    _fog.computeDepth = EditorGUILayout.Toggle(_fog.computeDepth, GUILayout.Width(40));
                    if (_fog.computeDepth)
                    {
                        EditorGUILayout.EndHorizontal();
                        EditorGUILayout.BeginHorizontal();
                        GUILayout.Label(new GUIContent("   Layer Mask", "Select which layers will be used for the Compute Depth option."), GUILayout.Width(130));
                        _fog.transparencyLayerMask = LayerMaskField(_fog.transparencyLayerMask);
                        EditorGUILayout.EndHorizontal();
                        EditorGUILayout.BeginHorizontal();
                        GUILayout.Label(new GUIContent("   Scope", "Select which kind of objects will be used for the Compute Depth option."), GUILayout.Width(130));
                        _fog.computeDepthScope = (COMPUTE_DEPTH_SCOPE)EditorGUILayout.EnumPopup(_fog.computeDepthScope);
                    }
                } else
                {
                    _fog.computeDepth = false;
                ...
                EditorGUILayout.BeginHorizontal ();
																...
																bool prev = _fog.renderBeforeTransparent;
																_fog.renderBeforeTransparent = EditorGUILayout.Toggle (_fog.renderBeforeTransparent);
																if (_fog.renderBeforeTransparent != prev)
																				GUIUtility.ExitGUI ();
																//GUILayout.Label("Debug", GUILayout.Width(60));
																//_fog.debugDepthPass = EditorGUILayout.Toggle(_fog.debugDepthPass, GUILayout.Width(20));
																EditorGUILayout.EndHorizontal ();

																if (_fog.renderBeforeTransparent)
																				GUI.enabled = false;
																EditorGUILayout.BeginHorizontal ();
																GUILayout.Label (new GUIContent ("Transparency Mode", "The transparency support mode. None, Screen Direct (blits a fog overlay over transparent objects directly to the screen) or Blend Pass (same but using a normal render pass which makes this compatible with VR)"), GUILayout.Width (130));
																TRANSPARENT_MODE prevMode = _fog.transparencyBlendMode;
																_fog.transparencyBlendMode = (TRANSPARENT_MODE)EditorGUILayout.EnumPopup (_fog.transparencyBlendMode);
																if (_fog.transparencyBlendMode != prevMode) {
																				GUIUtility.ExitGUI ();
																}
																EditorGUILayout.EndHorizontal ();


																if (_fog.transparencyBlendMode == TRANSPARENT_MODE.Blend) {
																				EditorGUILayout.BeginHorizontal ();
																				GUILayout.Label (new GUIContent ("   Blend Power", "Blending weight for the transparent objects."), GUILayout.Width (130));
																				_fog.transparencyBlendPower = EditorGUILayout.Slider (_fog.transparencyBlendPower, 0, 2f);
																				if (GUILayout.Button ("?", GUILayout.Width (20))) {
																								EditorUtility.DisplayDialog ("Transparency Blend Hint", "Transparency Blend works better if Volumetric Fog & Mist is the first image effect on the camera.", "Ok");
																				}
																				EditorGUILayout.EndHorizontal ();
																}
																GUI.enabled = true;
												...
												if (expandFogGeometrySection) {
																...
																_fog.density = EditorGUILayout.Slider (_fog.density, 0f, 1.25f);
																EditorGUILayout.EndHorizontal ();
																...
																_fog.noiseScale = EditorGUILayout.Slider (_fog.noiseScale, 0.2f, 25f);
																EditorGUILayout.EndHorizontal ();

                EditorGUILayout.BeginHorizontal();
                GUILayout.Label(new GUIContent("   Sparse", "Increase to make noise sparser."), GUILayout.Width(120));
                _fog.noiseSparse = EditorGUILayout.Slider(_fog.noiseSparse, -0.3f, 1f);
                EditorGUILayout.EndHorizontal();

                EditorGUILayout.BeginHorizontal();
                GUILayout.Label(new GUIContent("   Final Multiplier", "Final noise value multiplier."), GUILayout.Width(120));
                _fog.noiseFinalMultiplier = EditorGUILayout.Slider(_fog.noiseFinalMultiplier, 1f, 2f);
                EditorGUILayout.EndHorizontal();
                ...
                EditorGUILayout.BeginHorizontal ();
												...
												if (expandFogColorsSection) {
																...
																EditorGUILayout.BeginHorizontal ();
																GUILayout.Label (new GUIContent ("Lighting Model", "The lighting model used to calculate fog color. 'Legacy' is provided for previous version compatibility. 'Natural' uses ambient + light color. 'Single Light' excludes ambient color."), GUILayout.Width (120));
																_fog.lightingModel = (LIGHTING_MODEL)EditorGUILayout.EnumPopup (_fog.lightingModel);
																EditorGUILayout.EndHorizontal ();

																EditorGUILayout.BeginHorizontal ();
																GUILayout.Label (new GUIContent ("Light Intensity", "Intensity of the simulated directional light."), GUILayout.Width (120));
																if (_fog.lightingModel == LIGHTING_MODEL.Classic) {
																				_fog.lightIntensity = EditorGUILayout.Slider (_fog.lightIntensity, -1, 1);
																} else {
																				_fog.lightIntensity = EditorGUILayout.Slider (_fog.lightIntensity, 0, 3);
																}
																EditorGUILayout.EndHorizontal ();
												...
												if (IsFeatureEnabled (VolumetricFog.SKW_LIGHT_SCATTERING)) {
																...
																if (expandSunShaftsSection) {
																				...
																				if (_fog.lightScatteringEnabled && _fog.fogRenderer!=null && _fog.fogRenderer.sun == null) {
																								EditorGUILayout.BeginHorizontal ();
																				...
																				EditorGUILayout.BeginHorizontal ();
																				GUILayout.Label (new GUIContent ("Diffusion", "Spread or intensity of Sun light scattering. "), GUILayout.Width (120));
																				_fog.lightScatteringDiffusion = EditorGUILayout.Slider (_fog.lightScatteringDiffusion, 0, 1);
																				EditorGUILayout.EndHorizontal ();

																				EditorGUILayout.BeginHorizontal ();
																				GUILayout.Label (new GUIContent ("Shafts Intensity", "Intensity for the Sun shafts effect."), GUILayout.Width (120));
																				_fog.lightScatteringExposure = EditorGUILayout.Slider (_fog.lightScatteringExposure, 0, 0.2f);
																				EditorGUILayout.EndHorizontal ();
																				...
																				EditorGUILayout.BeginHorizontal ();
																				GUILayout.Label (new GUIContent ("   Samples", "Number of light samples used when Light Scattering is enabled. Reduce to increse performance."), GUILayout.Width (120));
												...
												if (IsFeatureEnabled (VolumetricFog.SKW_SUN_SHADOWS)) {
																...
																if (expandSunShadowsSection) {
																				...
																				if (_fog.hasCamera) {
																								...
																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("Layer Mask", "Select which layers will be used for the Sun Shadows option."), GUILayout.Width (120));
																								_fog.sunShadowsLayerMask = LayerMaskField (_fog.sunShadowsLayerMask);
																								EditorGUILayout.EndHorizontal ();
																								...
																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("Resolution", "Resolution of the shadow map texture. Sizes: 0=512, 1=1024, 2=2048, 3=4096, 4=8192."), GUILayout.Width (120));
																								...
																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("Shadow Cancellation", "Increase this value to reduce visibility of shadowed fog."), GUILayout.Width (120));
																								_fog.sunShadowsCancellation = EditorGUILayout.Slider (_fog.sunShadowsCancellation, 0f, 1f);
																								EditorGUILayout.EndHorizontal ();

																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("Shadow Map Update", "Realtime captures shadow map every frame. Discrete will bake shadow map only when Sun rotates or according to Force Update Interval parameter."), GUILayout.Width (120));
																								_fog.sunShadowsBakeMode = (SUN_SHADOWS_BAKE_MODE)EditorGUILayout.EnumPopup (_fog.sunShadowsBakeMode);
																								EditorGUILayout.EndHorizontal ();
					
																								if (_fog.sunShadowsBakeMode != SUN_SHADOWS_BAKE_MODE.Realtime) {
																												EditorGUILayout.BeginHorizontal ();
																												GUILayout.Label (new GUIContent ("Force Update Interval", "Updates shadow map texture after specified time interval (in seconds). Set this value to 0 to ignore shadow updating. Note that it will update whenever the Sun rotates."), GUILayout.Width (120));
																												_fog.sunShadowsRefreshInterval = EditorGUILayout.FloatField (_fog.sunShadowsRefreshInterval, GUILayout.Width (40));
																												EditorGUILayout.EndHorizontal ();
																								}

																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("Bias", "A threshold value to determine if a position is under shadow. A small value prevents self-shadowing."), GUILayout.Width (120));
																								_fog.sunShadowsBias = EditorGUILayout.FloatField (_fog.sunShadowsBias, GUILayout.Width (40));
																								EditorGUILayout.EndHorizontal ();

																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("Jittering", "Helps reducing shadow banding artifacts."), GUILayout.Width (120));
																								_fog.sunShadowsJitterStrength = EditorGUILayout.Slider (_fog.sunShadowsJitterStrength, 0, 0.5f);
																				} else {
																								if (_fog.sunShadows) {
																												EditorGUILayout.EndHorizontal ();
																												EditorGUILayout.BeginHorizontal ();
												...
												if (!fogVoidBoxEnabled && !fogVoidSphereEnabled) {
																...
																if (expandFogVoidSection) {
																				...
																				EditorGUILayout.BeginHorizontal ();
																				GUILayout.Label (new GUIContent ("Topology", "Shape of the void area."), GUILayout.Width (120));
																				_fog.fogVoidTopology = (FOG_VOID_TOPOLOGY)EditorGUILayout.EnumPopup (_fog.fogVoidTopology);
																				EditorGUILayout.EndHorizontal ();

																				if (_fog.fogVoidTopology == FOG_VOID_TOPOLOGY.Sphere) {
																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("Radius", "Radius of the void area."), GUILayout.Width (120));
																				...
																				} else if (_fog.fogVoidTopology == FOG_VOID_TOPOLOGY.Box) {
																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("Width", "Width of the void area."), GUILayout.Width (120));
																								_fog.fogVoidRadius = EditorGUILayout.Slider (_fog.fogVoidRadius, 0f, 2000f);
																								EditorGUILayout.EndHorizontal ();
                    ...
                    EditorGUILayout.BeginHorizontal();
                    GUILayout.Label(new GUIContent("Follow GameObject", "Assign a Game Object to follow its position automatically as center for the fog area."), GUILayout.Width(120));
                    _fog.fogAreaCenter = (GameObject)EditorGUILayout.ObjectField(_fog.fogAreaCenter, typeof(GameObject), true);
                    EditorGUILayout.EndHorizontal();

                    if (_fog.fogAreaCenter != null)
                    {
                        EditorGUILayout.BeginHorizontal();
                        GUILayout.Label(new GUIContent("   Follow Mode", "Specify if the fog area should follow this game object in 3D or only in XZ plane (useful for clouds)."), GUILayout.Width(120));
                        _fog.fogAreaFollowMode = (FOG_AREA_FOLLOW_MODE)EditorGUILayout.EnumPopup(_fog.fogAreaFollowMode);
                        EditorGUILayout.EndHorizontal();
                    }

                    EditorGUILayout.BeginHorizontal ();
																				GUILayout.Label (new GUIContent ("Topology", "Shape of the fog area."), GUILayout.Width (120));
																				_fog.fogAreaTopology = (FOG_AREA_TOPOLOGY)EditorGUILayout.EnumPopup (_fog.fogAreaTopology);
																				EditorGUILayout.EndHorizontal ();

                    if (_fog.hasCamera)
                    {

                        if (_fog.fogAreaTopology == FOG_AREA_TOPOLOGY.Sphere)
                        {
                            EditorGUILayout.BeginHorizontal();
                            GUILayout.Label(new GUIContent("Radius", "Radius of the fog area."), GUILayout.Width(120));
                            _fog.fogAreaRadius = EditorGUILayout.FloatField(_fog.fogAreaRadius, GUILayout.Width(80));
                            EditorGUILayout.EndHorizontal();

                            EditorGUILayout.BeginHorizontal();
                            GUILayout.Label(new GUIContent("Height", "Height of the fog area."), GUILayout.Width(120));
                            _fog.fogAreaHeight = EditorGUILayout.FloatField(_fog.fogAreaHeight, GUILayout.Width(80));
                            EditorGUILayout.EndHorizontal();
                        }
                        else
                        {
                            EditorGUILayout.BeginHorizontal();
                            GUILayout.Label(new GUIContent("Width", "Radius of the fog area."), GUILayout.Width(120));
                            _fog.fogAreaRadius = EditorGUILayout.FloatField(_fog.fogAreaRadius, GUILayout.Width(80));
                            EditorGUILayout.EndHorizontal();
                            ...
                            _fog.fogAreaDepth = EditorGUILayout.FloatField(_fog.fogAreaDepth, GUILayout.Width(80));
                            EditorGUILayout.EndHorizontal();
                        }
                    }
                    else if (_fog.fogAreaCenter == null)
                    {
                        EditorGUILayout.HelpBox("Position and resize this fog area using the transform position and scale.", MessageType.Info);
                    }
                    else if (_fog.fogAreaFollowMode == FOG_AREA_FOLLOW_MODE.RestrictToXZPlane)
                    {
                        ...
                        EditorGUILayout.BeginHorizontal ();
																				...
																				_fog.fogAreaFallOff = EditorGUILayout.Slider (_fog.fogAreaFallOff, 0.001f, 10f);
																				EditorGUILayout.EndHorizontal ();
																				EditorGUILayout.BeginHorizontal ();
																				GUILayout.Label (new GUIContent ("Sorting Mode", "Specify the sorting mode when rendering this fog area."), GUILayout.Width (120));
																				_fog.fogAreaSortingMode = (FOG_AREA_SORTING_MODE)EditorGUILayout.EnumPopup (_fog.fogAreaSortingMode);
																				if (_fog.fogAreaSortingMode == FOG_AREA_SORTING_MODE.Fixed) {
																								EditorGUILayout.EndHorizontal ();
																								EditorGUILayout.BeginHorizontal ();
																								GUILayout.Label (new GUIContent ("   Order", "Position in the sorting queue."), GUILayout.Width (120));
																								_fog.fogAreaRenderOrder = EditorGUILayout.IntField (_fog.fogAreaRenderOrder, GUILayout.Width (60));
																				}
																}
												}
												EditorGUILayout.EndHorizontal ();
												...
												if (expandOptimizationSettingsSection) {
																...
																EditorGUILayout.BeginHorizontal ();
																GUILayout.Label (new GUIContent ("Dithering", "Blends final fog color with a pattern to reduce banding artifacts. Use the slider to choose the intensity of dither."), GUILayout.Width (120));
...
//																EditorGUILayout.EndHorizontal ();
																...
																if (_fog.hasCamera) {
																				...
																				EditorGUILayout.BeginHorizontal ();
																				GUILayout.Label (new GUIContent ("Single Pass Stereo", "Specify the SPSR mode for this camera."), GUILayout.Width (120));
																				_fog.spsrBehaviour = (SPSR_BEHAVIOUR)EditorGUILayout.EnumPopup (_fog.spsrBehaviour);
																				EditorGUILayout.EndHorizontal ();
																...
																EditorGUILayout.BeginHorizontal ();
																GUILayout.Label (new GUIContent ("Fog Update Spread", "Spreads internal fog texture updates over several frames. A value of 1 will update fog immediately according to changes in lighting, Sun and other features. A value of 2 will distribute the workload in two frames, reducing CPU spikes. A greater value will span the update over more frames."), GUILayout.Width (120));
																_fog.updateTextureSpread = EditorGUILayout.IntSlider (_fog.updateTextureSpread, 1, 5);
																EditorGUILayout.EndHorizontal ();
												}
